...
preview_parts = [0,1,1,1,1,1]; // [0:Null,1:Extruder,2:Idler,3:Idler with brim,4:Strut,5:Axle,6:Tube Bracket]


// *****************************************************

if (preview_parts[1]) {
	translate([0,0,0]) rotate([0,0,270]) 
        difference() { 
            extruder_block(); 
            translate([0,0,1.5]) cube([200,200,3], center = true);
            }
};
if (preview_parts[2]) {
	translate([20,35,0]) rotate([0,0,90]) extruder_idler_608z(); //idler_and_flange();
};
if (preview_parts[3]) {
	translate([15,85,0]) idler_and_flange();
};
if (preview_parts[4]) {
	translate([70,35,0]) rotate([0,0,0]) support_strut();
};
if (preview_parts[5]) {
	translate([-10,40,0]) rotate([0,0,0]) axle_8mm();
};
if (preview_parts[6]) {
	translate([-60,80,0]) rotate([0,0,0]) tube_bracket();
};
...
module extruder_block() {
	difference() {
		union() {
			...
			translate([-15.5,-15.5,17.5]) cylinder(8.5,r1=4,r2=5, $fn=50);	// rev. 3
...
module bsp_push_fit() {
	...
	difference() {
		...
		translate([-2+x_nuj,-9,3]) rotate([0,0,20]) cube([10,10,26]);
            ...
            // screw in
            translate([11+x_nuj-3,7,18]) rotate([0,90,0]) cylinder(6, r=0.5, $fn=50);
            translate([11+x_nuj-3,7,12]) rotate([0,90,0]) cylinder(6, r=0.5, $fn=50);
            translate([11+x_nuj-3,7+3,15]) rotate([0,90,0]) cylinder(6, r=0.5, $fn=50);
            translate([11+x_nuj-3,7-3,15]) rotate([0,90,0]) cylinder(6, r=0.5, $fn=50);
}
